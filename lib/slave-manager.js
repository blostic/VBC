...
var SlaveManager = function (listenPort) {
    ...
    this.serverSocket = io.listen(listenPort);
    ...
    this.serverSocket.on('connection', function(socket) {
        ...
        socket.on('task_completed', function(msg) {
            ...
            if (pkg.completed === pkg.total) {
                console.log('package completed: ' + taskInfo.packageId);
            ...
            var slave = manager.busySlaves.splice(slaveIdx, 1);
        ...
        socket.on('disconnect', function() {
            ...
            if (idx >= 0) {
                ...
                console.log('slave disconnected, task ' + slave.taskId
                            + ' added at the start of task queue');
            }
            
            // this should never happen
            console.log('something weird happened, closed socket should be in '
...
SlaveManager.prototype = {
    _debugPrint: function(funcName) {
        console.log('slave manager: ' + funcName + ' called');
        console.log('- ' + this.taskQueue.length + ' tasks in queue');
        console.log('- ' + this.idleSlaves.length + ' idle slaves');
        console.log('- ' + this.busySlaves.length + ' busy slaves');
    },
    _assignTasks: function() {
        this._debugPrint('_assignTasks');
        ...
        while (this.taskQueue.length > 0 && this.idleSlaves.length > 0) {
            ...
            var remoteAddress = slave.socket.handshake.address;
            console.log('task ' + slave.taskId + ' assigned to '
    ...
    enqueue : function (tasks) {
        ...
        if (tasks.length === 0) {
            console.log('warning: empty array passed to SlaveManager.enqueue()');
