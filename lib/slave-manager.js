...
var SlaveManager = function (listenPort, debug) {
    ...
    this.serverSocket.on('connection', function(socket) {
        ...
        socket.on('task_reply', function(msg) {
            manager._markTaskAsCompleted(msg.task_id, msg.result);
        ...
        socket.on('disconnect', function() {
            var addr = socket.handshake.address;
            manager._debugLog('slave %s:%d disconnected', addr.address, addr.port);

            var findBySlaveSocket = function(slave) {
                return slave.socket == socket;
            };

            var idx = _.findIndex(manager.idleSlaves, findBySlaveSocket);
            if (idx >= 0) {
                // idle slave disconnected - ok
                manager.idleSlaves.splice(idx, 1);
                manager._debugLog('idle slave disconnected');
                return;
            }

            idx = _.findIndex(manager.busySlaves, findBySlaveSocket);
            if (idx >= 0) {
                // busy slave disconnected - try to recover
                var slave = manager.busySlaves.splice(idx, 1)[0];
                var taskInfo = manager.tasks[slave.taskId];
                taskInfo.task.status = 'scheduled';
                manager.taskQueue.unshift(taskInfo);
...
SlaveManager.prototype = {
    ...
    _markTaskAsCompleted: function(taskId, result) {
        ...
        if (pkg === undefined) {
            console.error('task %j is assigned to invalid package ', taskId
                          + taskInfo.packageId);
            return;
        }
        ...
        this._updateDequeuedDeferreds(taskInfo.task);
    },
    ...
    _updateDequeuedDeferreds: function(completedTask) {
        var manager = this;

        if (completedTask === undefined) {
            for (var taskId in this.finishedTasks) {
                this._updateDequeuedDeferreds(this.finishedTasks[taskId].task);
            }
            return;
        }

        this._debugLog('updateDequeuedDeferreds: taskId = %s', completedTask.id);

        var dequeuesPerTask = 0;
        _.forEach(this.dequeuedDeferreds, function(deferredInfo) {
            var idx = deferredInfo.taskIds.indexOf(completedTask.id);
            if (idx >= 0) {
                deferredInfo.taskIds.splice(idx, 1)[0];
                deferredInfo.completedTasks.push(completedTask);

                delete manager.completedTasks[completedTask.id];
