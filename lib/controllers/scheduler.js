...
var scheduler = new Scheduler({
    jobTypes : job_types,
    slaveManager : new SlaveManager(9001, true)
});
                ...
                Task.countQ({ job: job.id, status: "completed"})
                    .then(function (completed){
                        res.json({
                            ...
                            data          : job.data,
                            result        : job.result,
                            progress      : (total - scheduler.slaveManager.taskQueue.length) / total,
                            totalTasks    : total,
                            finishedTasks : total - scheduler.slaveManager.taskQueue.length
                        });
                    })
                    .done()
            })
